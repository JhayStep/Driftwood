    ...
    match cli.cmd {
        Cmd::Run { peer } => {
            if let Some(peer_addr) = peer {
                let peer_store = storage::Storage {
                    db: store.db.clone(),
                };

                let node_id = cli.node.clone();

                tokio::spawn(async move {
                    if let Err(e) = gossip::connect(&peer_addr, &node_id, peer_store).await {
                        tracing::warn!(?e, "peer connection failed");
                    }
                });
            }

            gossip::listen(&cli.listen, &cli.node, store).await?;
        }

        Cmd::Put { doc, key, val } => {
            use crdt_core::LwwMap;

            let mut m: LwwMap<String, String> = if let Some(bytes) = store.get_doc(&doc)? {
                bincode::deserialize(&bytes)?
            } else {
                LwwMap::new(cli.node.clone())
            };
        ...
        Cmd::Get { doc, key } => {
            if let Some(bytes) = store.get_doc(&doc)? {
                let m: crdt_core::LwwMap<String, String> = bincode::deserialize(&bytes)?;
